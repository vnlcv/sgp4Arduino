...
const unsigned long TIMER_INTERVAL_MS = 50; // Timer interval in milliseconds
...
static unsigned long hardcodedUnixTime = 1731334602; // Fallback Unix time
...
void loop() {
  ...
  if (!homingComplete) {
    ...
    if (millis() - lastUpdate >= TIMER_INTERVAL_MS) {  // Update every TIMER_INTERVAL_MS
      ...
      psi_d2_degrees = int(angles.psi_d2) % 360;  // Change target angle
